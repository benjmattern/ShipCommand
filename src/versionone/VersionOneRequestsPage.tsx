...
export function VersionOneRequestsPage() {
  ...
  const displayedRequests = useMemo(() => sortVersionOneRequests(
    filterVersionOneRequests(requests, search, status, priority, owner), sortField, sortDirection,
  ), [requests, search, status, priority, owner, sortField, sortDirection]);
  const filtering = Boolean(search.trim() || status || priority || owner);
      ...
      {result && <>
        ...
        {result.recordCount === 0 ? <p className="empty-state standalone">No VersionOne Requests were found.</p> : <>
          <div className="versionone-filters request-filters">
            ...
            <span>{displayedRequests.length} of {result.recordCount} shown</span>
          </div>
